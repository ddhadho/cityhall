...
mod tests {
    ...
    fn test_bloom_filter_time_series() {
        ...
        println!(
            "Rejection rate: {:.1}% ({}/1000)",
            rejection_rate * 100.0,
            rejections
        );

        assert!(rejection_rate > 0.90, "Should reject >90% of absent keys");

        println!(
            "✓ Time-series test: {:.1}% rejected",
            rejection_rate * 100.0
        );
    }

...
use super::format::{Footer, Header, IndexEntry, HEADER_SIZE};
///
...
impl SsTableWriter {
    ...
    fn flush_block(&mut self) -> Result<()> {
        ...
        let first_key = self
            .block_builder
            .first_key()
            .ok_or_else(|| crate::StorageError::InvalidFormat("Block has no first key".into()))?
            .to_vec();
...
mod tests {
    ...
    fn test_writer_many_entries() -> Result<()> {
        ...
        let metadata = std::fs::metadata(&path)?;
        println!(
            "Large SSTable: {} bytes, {} blocks",
            metadata.len(),
            writer.header.num_blocks
        );

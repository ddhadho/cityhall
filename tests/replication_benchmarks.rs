...
async fn bench_offline_recovery_catchup_speed() {
    println!("\n╔══════════════════════════════════════════════════════════╗");
    println!("║  BENCHMARK: Offline Recovery & Catch-Up Speed           ║");
    println!("╚══════════════════════════════════════════════════════════╝\n");
    ...
    println!("📝 Phase 1: Writing initial data (100 entries)...");
    for i in 0..100 {
        let entry = Entry {
    ...
    println!("\n📝 Phase 3: SIMULATING OFFLINE PERIOD");
    ...
    println!(
        "   ✓ Leader wrote {} entries while replica offline",
        entries_missed
    );
    println!(
        "   Leader now has {} closed segments",
    ...
    println!("\n🔄 Phase 4: REPLICA RECOVERY - Measuring catch-up speed...");
    ...
    println!("╠══════════════════════════════════════════════════════════╣");
    println!(
        "║  Entries missed during offline: {:>6}                   ║",
        entries_missed
    );
    println!(
        "║  Segments to catch up:          {:>6}                   ║",
        segments_synced
    );
    println!(
        "║  Catch-up time:                 {:>6.2?}                ║",
    ...
    println!(
        "║  Catch-up throughput:           {:>6.0} entries/sec     ║",
        catchup_throughput
    );
    println!(
        "║  Final replica segment:         {:>6}                   ║",
        final_replica_segment
    );
    println!(
        "║  Total entries applied:         {:>6}                   ║",
    ...
    assert!(
        catchup_time < Duration::from_secs(30),
        "Catch-up should complete within 30 seconds"
    );
    assert!(
        catchup_throughput > 10.0,
        "Should sync at least 10 entries/sec"
    );

    println!("✅ BENCHMARK PASSED: Offline recovery within acceptable limits\n");
}
...
async fn bench_multiple_replicas_convergence() {
    println!("\n╔══════════════════════════════════════════════════════════╗");
    println!("║  BENCHMARK: Multiple Replicas Convergence               ║");
    println!("╚══════════════════════════════════════════════════════════╝\n");
    ...
    println!("📝 Writing {} entries to leader...", ENTRIES_COUNT);
    ...
    println!("   ✓ Created {} segments", segments_count);
    ...
    println!("\n🔄 Starting {} replicas concurrently...", NUM_REPLICAS);
    ...
    let max_time = results.iter().map(|(_, t, _, _)| *t).max().unwrap();

    println!("╠══════════════════════════════════════════════════════════╣");
    println!(
        "║  Overall convergence time:  {:>6.2?}                    ║",
        overall_time
    );
    println!(
        "║  Average sync time:         {:>6.2} seconds            ║",
        avg_time
    );
    println!(
        "║  Slowest replica:           {:>6.2?}                    ║",
    ...
    assert!(
        overall_time < Duration::from_secs(10),
        "All replicas should converge within 10 seconds"
    );

    println!("✅ BENCHMARK PASSED: All replicas converged consistently\n");
}
...
async fn bench_crash_recovery_with_state_persistence() {
    println!("\n╔══════════════════════════════════════════════════════════╗");
    println!("║  BENCHMARK: Crash Recovery & State Persistence          ║");
    println!("╚══════════════════════════════════════════════════════════╝\n");
    ...
    // === RUN 1: Initial sync ===
    println!("\n🔄 RUN 1: Initial sync before crash...");
...

    {
        ...
        println!(
            "   ✓ Synced {} segments ({} entries)",
            synced_segment, entries_applied
        );
        println!("   💥 SIMULATING CRASH (dropping agent)...");
    ...
    assert!(
        replica_state_path.exists(),
        "State file should exist after crash"
    );

    // Leader writes more data while replica is "crashed"
    println!("\n📝 Leader writes more data while replica crashed...");
    {
        let mut wal = leader_wal.write();
        for i in 150..300 {
            let entry = Entry {
    ...
    println!("   ✓ Leader wrote 150 more entries");

    // === RUN 2: Recovery after crash ===
    println!("\n🔄 RUN 2: Replica restarts and recovers...");
...

    {
        ...
        println!(
            "   ✓ Recovered state: last_synced_segment = {}",
            recovered_segment
        );
        ...
        assert!(
            agent.state().last_synced_segment >= 2,
            "Should have synced up to at least segment 2 after recovery"
        );
        let final_entries = agent.state().total_entries_applied;

        println!(
            "   ✓ Synced {} more segments after recovery",
            segments_after_recovery
        );
        println!("   ✓ Final entry count: {}", final_entries);

        assert_eq!(
            final_entries, 300,
            "Should have all 300 entries after recovery"
        );
    }
    ...
    println!("╠══════════════════════════════════════════════════════════╣");
    println!(
        "║  Run 1 (before crash):      {:>6.2?}                    ║",
        run1_time
    );
    println!(
        "║  Run 2 (after recovery):    {:>6.2?}                    ║",
        run2_time
    );
    println!("║  State persisted:           ✓ YES                       ║");
    println!("║  Data loss:                 ✓ NONE                      ║");
    println!(
        "║  Recovery time:             {:>6.2?}                    ║",
        run2_time
    );
    ...
    assert!(
        run2_time < Duration::from_secs(15),
        "Recovery should complete within 15 seconds"
    );

    println!("✅ BENCHMARK PASSED: State persisted, zero data loss\n");
}

/// Benchmark: Replication Latency - Measure time from write to replica
///
/// Measures the end-to-end latency: write to leader → available on replica
#[tokio::test]
async fn bench_replication_latency() {
    println!("\n╔══════════════════════════════════════════════════════════╗");
    println!("║  BENCHMARK: Replication Latency (Write → Replica)       ║");
    println!("╚══════════════════════════════════════════════════════════╝\n");
    ...
    println!(
        "📊 Measuring replication latency over {} cycles...\n",
        CYCLES
    );
    ...
    println!("✅ BENCHMARK PASSED: Latency within acceptable range\n");
}

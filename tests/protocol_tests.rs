...
async fn test_multiple_requests_same_connection() {
    ...
    // Request 1
    send_request(&mut stream, &SyncRequest::GetSegment { segment_number: 1 }).await.unwrap();
    let response1 = recv_response(&mut stream).await.unwrap().unwrap();
    
    // Request 2
    send_request(&mut stream, &SyncRequest::GetSegment { segment_number: 2 }).await.unwrap();
    let response2 = recv_response(&mut stream).await.unwrap().unwrap();
    
    // Verify both responses
    match (response1, response2) {
        (
            SyncResponse::SegmentData { segment_number: n1, .. },
            SyncResponse::SegmentData { segment_number: n2, .. }
        ) => {
            assert_eq!(n1, 1);
            assert_eq!(n2, 2);
        }

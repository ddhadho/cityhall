...
fn test_wal_cleanup() {
    ...
    println!(
        "  Writing {} more entries to trigger flush...",
        entries_needed
    );
    ...
    assert!(
        wal_size_after < 150 * 1024 * 1024,
        "WAL should not exceed 150MB: got {} bytes",
        wal_size_after
    );
}
...
fn test_wal_recovery_after_cleanup() {
    ...
    println!("\nPhase 4: Recovering...");
    {
        ...
        assert!(
            found_count >= 900,
            "Should recover most entries from WAL: got {}/1000",
            found_count
        );
